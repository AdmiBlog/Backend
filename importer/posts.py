...
for i in os.listdir("importer/_posts"):
    with open("importer/_posts/" + i, "r", encoding="utf-8") as f:
        textContent = f.read()
        ...
        res.append(
            {
                ...
                "lastUpdatedTime": data.get("updated",data.get("date")).timestamp(),
                "slug": i.strip(".md"),
                "bannerImg": data.get("cover"),
                "plainContent": plainContent,
                "wordCount": wordCount,
                "coverFit": data.get("top_img"),
            }
        )
myclient[os.getenv("DB_NAME") or "AdmiBlog"]["Posts"].delete_many({})
myclient[os.getenv("DB_NAME") or "AdmiBlog"]["Posts"].insert_many(res)
